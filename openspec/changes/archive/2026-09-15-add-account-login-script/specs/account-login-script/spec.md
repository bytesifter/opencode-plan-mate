...
脚本 SHALL 检测并防护账号 HOME 路径污染：`~` 展开基于的可信 home 目录若含 `.arkcli-accounts` 片段（脚本自管目录标记），SHALL 判定为污染并回退到真实用户 home。启动后 SHALL 校验每个解析出的账号 HOME：若仍含嵌套 `.arkcli-accounts` 片段，SHALL 打印警告并以非零退出码中止，SHALL NOT 在嵌套路径上执行登录。
...
脚本 SHALL 执行 `arkcli auth login --no-browser`，从 stdout 解析 `authorize_url`，使用系统默认浏览器打开；待用户在浏览器完成授权后，脚本 SHALL 在终端提示用户粘贴 base64 授权码，并以 `arkcli auth login --no-browser --code <code>` 完成登录。授权码获取失败或超时 SHALL 标注账号失败。
